...
func TestMsgDecode(t *testing.T) {
	...
	msg, err := types.NewMsgCreateValidator(valAddr1, pk1, coinPos, types.Description{}, commission1, sdk.OneInt(), "standing")
	require.NoError(t, err)
...
func TestMsgCreateValidator(t *testing.T) {
	...
	for _, tc := range tests {
		...
		msg, err := types.NewMsgCreateValidator(tc.validatorAddr, tc.pubkey, tc.bond, description, tc.CommissionRates, tc.minSelfDelegation, "standing")
		require.NoError(t, err)
		if tc.expectPass {
