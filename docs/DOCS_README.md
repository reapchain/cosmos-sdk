...
If you want to open a PR in Cosmos SDK to update the documentation, please follow the guidelines in [`CONTRIBUTING.md`](https://github.com/reapchain/cosmos-sdk/tree/master/CONTRIBUTING.md#updating-documentation).
...
- Each `docs/<locale>/` folder must follow the same folder structure within `docs/`, but only content in the following folders needs to be translated and included in the respective `docs/<locale>/` folder:
    ...
    - `docs/migrations/`
    - `docs/run-node/`
- Each `docs/<locale>/` folder must also have a `README.md` that includes a translated version of both the layout and content within the root-level [`README.md`](https://github.com/reapchain/cosmos-sdk/tree/master/docs/README.md). The layout defined in the `README.md` is used to build the homepage.
...
[master](https://github.com/reapchain/cosmos-sdk/tree/master/docs).
...
Then, edit the `swagger.yaml` manually; it is found [here](https://github.com/reapchain/cosmos-sdk/blob/master/client/lcd/swagger-ui/swagger.yaml)
...
much as possible with its [counterpart in the Tendermint Core repo](https://github.com/reapchain/reapchain-core/blob/v0.34.0/docs/DOCS_README.md).
